...
GENERIC_COMPANIES = {
    "codesignal", "leetcode", "neetcode", "hello", "mail", "comms", "unknown"
}

# IMPORTANT:
# "next stage" removed from here because it was creating fake interview count.
# Interview will count only when the email has real interview wording.
INTERVIEW_STRICT = [
    "interview invite",
...
def load_data():
    url = (
        f"https://docs.google.com/spreadsheets/d/{sheet_id(SHEET_URL)}/gviz/tq"
...
def real_assessment(text):
    context = [
        "application", "role", "position", "recruit", "hiring",
        "graduate", "engineer", "analyst", "software"
    ]
...
def status_for(row):
    ...
    # Agar old sheet me "Interview / Scheduling" stored hai but real interview phrase nahi mila,
    # usko update treat karo, interview nahi.
...
def prepare(df):
    ...
    df.loc[df["Received Datetime"].isna(), "Received Datetime"] = pd.to_datetime(
        df["Received Date"],
        errors="coerce"
    )
    ...
    df["Action Needed"] = df["Status"].isin([
        "Recruiter / Follow-up",
        "Online Assessment",
        "Interview / Next Stage"
    ])
    ...
    df["Priority"] = df["Status"].map({
        "Interview / Next Stage": "High",
        "Online Assessment": "Medium",
        "Recruiter / Follow-up": "Medium"
    }).fillna("Normal")

    df["Thread Type"] = df["Status"].map({
...
def filter_data(df):
    ...
    if st.sidebar.button("Refresh now", use_container_width=True, key="refresh_now_button"):
        st.cache_data.clear()
        st.rerun()
    ...
    min_date = (
        df["Received Datetime"].min().date()
        if pd.notna(df["Received Datetime"].min())
        else date.today() - timedelta(days=30)
    )

    max_date = (
        df["Received Datetime"].max().date()
        if pd.notna(df["Received Datetime"].max())
        else date.today()
    )
    ...
    statuses = st.sidebar.multiselect(
        "Status",
        [s for s in STATUS_ORDER if s in set(df["Status"])],
        key="status_filter"
    )
...
def show_table(df, key_prefix):
    ...
    display = df[cols].rename(
        columns={
            "Company Name": "Company",
            "Job Role": "Role",
            "Gmail Link": "Open Email"
        }
    )
    ...
    st.dataframe(
        ...
        column_config={
            "Open Email": st.column_config.LinkColumn(
                "Open Email",
                display_text="Open"
            )
        }
    ...
    st.download_button(
        ...
        f"job_applications_{key_prefix}.csv",
        "text/csv",
...
def main():
    ...
    with c1:
        metric("Total", len(filtered), "clean rows")

    with c2:
        metric(
            "Applied",
            int((filtered["Status"] == "Application Applied").sum()) if not filtered.empty else 0,
            "confirmations"
        )
    ...
    with c3:
        metric(
            "Updates",
            int((filtered["Status"] == "Application Update").sum()) if not filtered.empty else 0,
            "updates"
        )
    ...
    with c4:
        metric(
            "Assessments",
            int((filtered["Status"] == "Online Assessment").sum()) if not filtered.empty else 0,
            "tests"
        )
    ...
    with c5:
        metric(
            "Interviews",
            int((filtered["Status"] == "Interview / Next Stage").sum()) if not filtered.empty else 0,
            "strict invites only"
        )
    ...
    with c6:
        metric(
            "Rejected",
            int((filtered["Status"] == "Rejected").sum()) if not filtered.empty else 0,
            "closed"
        )
    ...
    with tabs[0]:
        if filtered.empty:
            ...
            counts["Order"] = counts["Status"].map(
    ...
    with tabs[1]:
        show_table(
            filtered[filtered["Action Needed"]],
            "followups"
        )
    ...
    with tabs[3]:
        if filtered.empty:
            ...
            daily = (
                filtered.dropna(subset=["Received Datetime"])
                .assign(Day=lambda z: z["Received Datetime"].dt.date)
                .groupby(["Day", "Status"], as_index=False)
                .size()
                .rename(columns={"size": "Count"})
            )
